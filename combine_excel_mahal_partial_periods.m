...
refsample = [xa',rsamplesize];
save([myResult,ShimmerData(k).filename{1}(1:end-5),'-rsample.mat'],'refsample');
%--------------------------------------------------------------------------
%partial periods: 100%,95%,90%,85%,80%,75%
subdirs = {'p100','p095','p090','p085','p080','p075'};
samplesize = zeros(6,size(ShimmerData,2));
for j=4%19%:size(ShimmerData,2)
    ...
    for kp=1:size(samplesize,1)
        ...
        saveas(gcf,[destpath,ShimmerData(j).filename{1}(1:end-5),'-',subdirs{kp}(2:end),'-',num2str(samplesize(kp,j)),'-FallProb.pdf'])
    end
    %-------------------------[plot sensor data]---------------------------
    ...
    title(ShimmerData(j).filename{1}(1:end-5));
    legend(h,{'X axis','Y axis','Z axis'},'Location','north');
...
save([myResult,'SampleSizes.mat'],'samplesize');