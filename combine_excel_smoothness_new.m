...
myResult = 'D:\Results\walk\mahal\3\';
...
for j=1:size(ShimmerData,2)
    ...
    %xdif = diff(ShimmerData(j).data(:,1));
    %ydif = diff(ShimmerData(j).data(:,2));
    %zdif = diff(ShimmerData(j).data(:,3));
    %fd = sqrt(xdif.^2+ydif.^2+zdif.^2);
    %fd = [0;fd];
    %dh = plot([data(:,dcol),fd]);
    dh = plot(data(:,dcol));
    ...
    for k=1:niter+1
        ...
        %matchv(k+mxa) = exp(-md/10);
    end
    h = plot(data(tg_pos:size(data,1)-tg_pos,dcol));
    xlabel ('time');  
    ylabel ('sensor data');
    title(ShimmerData(j).filename{1}(1:end-5));
    ...
    plot(matchv(tg_pos:size(data,1)-tg_pos));
    axis([2,length(tg_pos:size(data,1)-tg_pos),0,1]);
    xlabel ('time');  
    ylabel ('Match Level');
    title(ShimmerData(j).filename{1}(1:end-5));
    saveas(gcf,[myResult,ShimmerData(j).filename{1}(1:end-5),'-matching.pdf'])
    input('next file');
end
save('SamplewidthSize.mat','samplesize');
%chi square computing
% chisq = zeros(1,size(data,1));
% for df = 1:500
% C(df,:) = chi2inv(1-[0.995 0.99 0.975 0.95 0.9 0.1 0.05 0.025 0.01 0.005],df);
% end
% for i = 1:size(refdata,2);
%     for m = 1:size(ShimmerData,2);
%         chisq(m) = sum((((data(m))-(refdata(i)))^2)/(refdata(i)));   
%         m = m+1;
%     end
%     i = i+1;
% end